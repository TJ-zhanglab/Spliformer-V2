...
def masked_mse_loss(pred, target):
    # Create weights: if target > 0, weight = 1.0; else weight = 0.1 (or 0)
    weights = torch.where(target > 0, 1.0, 0.1)
    mse_loss_fn = MSELoss(reduction='none')  # 创建 MSELoss 实例
...
#     def __init__(self, alpha=0.75, gamma=2, majority_weight=0.8, minority_weight=0.2):
#         """
#         Args:
#             alpha (float): Weighting factor for the minority class (class 1).
#             gamma (float): Focusing parameter to adjust the rate at which easy examples are down-weighted.
#             majority_weight (float): Weight for the majority class loss.
#             minority_weight (float): Weight for the minority class loss.
#         """
...
#         """
#         Args:
#             inputs (torch.Tensor): Model predictions (logits, shape [batch_size, 1]).
#             targets (torch.Tensor): True labels (0 or 1, shape [batch_size]).
#         """
#         # Apply sigmoid to logits to get probabilities for the positive class
#         probs = torch.sigmoid(inputs)
        
#         # Compute binary cross-entropy loss
#         BCE_loss = F.binary_cross_entropy_with_logits(inputs, targets.float(), reduction='none')
        
#         # Get the true probabilities (p_t) for each sample
#         probs_true = probs * targets + (1 - probs) * (1 - targets)  # p_t
...
#         # Final loss: element-wise focal loss
#         loss = focal_weight * alpha_weight * BCE_loss
        
#         # Separate loss computation for minority and majority classes
#         minority_loss = loss[targets == 1].mean() if (targets == 1).sum() > 0 else 0
#         majority_loss = loss[targets == 0].mean() if (targets == 0).sum() > 0 else 0

#         # Combine losses with respective weights
#         total_loss = self.minority_weight * minority_loss + self.majority_weight * majority_loss
...
class SegmentNT(EsmPreTrainedModel):
    def __init__(self, config):
        ...
        self.fc = nn.Linear(in_features=embed_dim, out_features=6 * 1 * self.num_features) #2class>3class 6 * 2 * self.num_features > 6 * 3 * self.num_features 
    ...
    def forward(
        ...
        sequence_output = outputs[0] #torch.Size([1, 2001, 1024])
        ...
        logits = torch.reshape(logits, (x.shape[0], x.shape[1] * 6, self.num_features, 1)) 
        

        
        logits=torch.sigmoid(logits)
        logits_a1=logits[:,:720,:,:]
